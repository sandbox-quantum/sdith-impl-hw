...
parameter VCD_NAME =    (TV_type == 0 && TYPE == "S")?"compute_S_tb_0.vcd" :
                        (TV_type == 1 && TYPE == "S")?"compute_S_tb_1.vcd" :
                        (TV_type == 2 && TYPE == "S")?"compute_S_tb_2.vcd" :
                        (TV_type == 0 && TYPE == "P")?"compute_P_tb_0.vcd" :
                        (TV_type == 1 && TYPE == "P")?"compute_P_tb_1.vcd" :
                                                      "compute_P_tb_2.vcd";
...
parameter X_NAME =   (TV_type == 0)? "X_L1_0.mem":
                     (TV_type == 1)? "X_L1_1.mem":
                                                "X_L1_2.mem";
mem_single #(.WIDTH(8), .DEPTH(M/D), .FILE(X_NAME)) 
 X_val
 (
 .clock(i_clk),
 .data(0),
...
parameter Q_NAME =   (TV_type == 0)? "Q_L1_0.mem":
 ...
 parameter Q =     (PARAMETER_SET == "L1")? Q_NAME:
                   (PARAMETER_SET == "L3")? "f_poly_L3.mem":
                   (PARAMETER_SET == "L5")? "f_poly_L5.mem":
                                            "f_poly_L1.mem";                                           
